...
func (a *Attr) Int64() (int64, error) {
	...
	if !ok {
		return 0, fmt.Errorf("hcl: cannot read attribute %q as literal", a.K)
	}
	i, err := strconv.ParseInt(lit.V, 10, 64)
	if err != nil {
		return 0, fmt.Errorf("hcl: cannot read attribute %q as integer: %w", a.K, err)
	}
	return i, nil
}
...
func (a *Attr) Bool() (bool, error) {
	lit, ok := a.V.(*LiteralValue)
	if !ok {
		return false, fmt.Errorf("hcl: cannot read attribute %q as literal", a.K)
	}
	b, err := strconv.ParseBool(lit.V)
	if err != nil {
		return false, fmt.Errorf("hcl: cannot read attribute %q as bool: %w", a.K, err)
	}
	return b, nil
}
...
func (a *Attr) Ref() (string, error) {
	ref, ok := a.V.(*Ref)
	if !ok {
		return "", fmt.Errorf("hcl: cannot read attribute %q as ref", a.K)
	}
	return ref.V, nil
}
...
func (a *Attr) Strings() ([]string, error) {
	...
	if !ok {
		return nil, fmt.Errorf("hcl: attribute %q is not a list", a.K)
	}
	out := make([]string, 0, len(lst.V))
	for _, item := range lst.V {
		sv, err := StrVal(item)
		if err != nil {
			return nil, fmt.Errorf("hclspec: failed parsing item %q to string: %w", item, err)
		}
		out = append(out, sv)
	}
	return out, nil
...
func (a *Attr) Bools() ([]bool, error) {
	lst, ok := a.V.(*ListValue)
	if !ok {
		return nil, fmt.Errorf("hclspec: attribute %q is not a list", a.K)
	}
...
func StrVal(v Value) (string, error) {
	...
	if !ok {
		return "", fmt.Errorf("hclspec: expected %T to be LiteralValue", v)
	}
...
func BoolVal(v Value) (bool, error) {
	lit, ok := v.(*LiteralValue)
	if !ok {
		return false, fmt.Errorf("hclspec: expected %T to be LiteralValue", v)
	}
	b, err := strconv.ParseBool(lit.V)
	if err != nil {
		return false, fmt.Errorf("hclspec: failed parsing %q as bool: %w", lit.V, err)
	}
	return b, nil
}
