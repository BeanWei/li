...
func WithTypes(typeSpecs []*spec.TypeSpec) Option {
	newCtx := func() *hcl.EvalContext {
		ctx := &hcl.EvalContext{
			Variables: make(map[string]cty.Value),
			Functions: make(map[string]function.Function),
...
func rawExprImpl() function.Function {
	return function.New(&function.Spec{
		...
		Impl: func(args []cty.Value, retType cty.Type) (cty.Value, error) {
			t := &spec.RawExpr{X: args[0].AsString()}
...
func typeFuncSpec(typeSpec *spec.TypeSpec) function.Function {
	spec := &function.Spec{
...
func typeFuncSpecImpl(s *function.Spec, typeSpec *spec.TypeSpec) function.ImplFunc {
	return func(args []cty.Value, retType cty.Type) (cty.Value, error) {
		...
		for _, attr := range typeFuncArgs(typeSpec) {
			...
			if attr.Kind == reflect.Slice {
				...
				t.Attrs = append(t.Attrs, &spec.Attr{K: attr.Name, V: lst})
				break
			}
			if len(args) == 0 {
				break
			...
			t.Attrs = append(t.Attrs, &spec.Attr{K: attr.Name, V: v})
		}
...
func typeFuncArgs(s *spec.TypeSpec) []*spec.TypeAttr {
	var args []*spec.TypeAttr
...
func typeFuncReqArgs(s *spec.TypeSpec) []*spec.TypeAttr {
	var args []*spec.TypeAttr
	for _, arg := range typeFuncArgs(s) {
		if arg.Required {
