...
func evalCtx(ctx *hcl.EvalContext, f *hcl.File) (*hcl.EvalContext, error) {
	...
	if !ok {
		return nil, fmt.Errorf("lihcl: expected an hcl body")
	}
...
// ctySchemaLit is a cty.Capsule type the encapsulates a spec.LiteralValue.
var (
	ctySchemaLit = cty.CapsuleWithOps("lit", reflect.TypeOf(spec.LiteralValue{}), &cty.CapsuleOps{
		...
		ConversionFrom: func(src cty.Type) func(interface{}, cty.Path) (cty.Value, error) {
			...
			return func(i interface{}, path cty.Path) (cty.Value, error) {
				lit, ok := i.(*spec.LiteralValue)
				if !ok {
					return cty.Value{}, fmt.Errorf("lihcl: expected *spec.LiteralValue got %T", i)
				}
	...
	ctyTypeSpec = cty.Capsule("type", reflect.TypeOf(spec.Type{}))
	ctyRawExpr  = cty.Capsule("raw_expr", reflect.TypeOf(spec.RawExpr{}))
)
