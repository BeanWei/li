...
func (tx *Tx) init() {
	tx.FlowDefinition = NewFlowDefinitionClient(tx.config)
	tx.FlowDeployment = NewFlowDeploymentClient(tx.config)
	tx.FlowInstance = NewFlowInstanceClient(tx.config)
	tx.FlowInstanceData = NewFlowInstanceDataClient(tx.config)
	tx.FlowNodeInstance = NewFlowNodeInstanceClient(tx.config)
	tx.FlowNodeInstanceLog = NewFlowNodeInstanceLogClient(tx.config)
}
...
// applies a query, for example: FlowDefinition.QueryXXX(), the query will be executed
